	...
	void proc(){
		...
		if(return_Choice.equals("0")){
			...
			if(arr.size() != 0){	
				...
				do{
					System.out.print("[도서반납]반납할 도서의 일련번호 입력 ([0]메인으로): ");
					book_Id = input.next();
			...
			do{
				System.out.print("[반납취소]사번 입력 (0:메인으로): ");
				member_Id = input.next();
				if(member_Id.equals("0")) return;
			...
			do{
				System.out.print("[반납취소]책의 일련번호 입력 (0:메인으로): ");
				book_Id = input.next();
				if(book_Id.equals("0")) return;
				arr = dao.return_Check(member_Id, book_Id);
				// if(arr.size() != 0){					
					// System.out.print("[반납취소]반납일자 [1]수기 [2]오늘(자동입력) [0]메인으로: ");
					// date_Input = input.next();
					// if(date_Input.equals("0")){
						// return;
					// }else if(date_Input.equals("1")){
						// boolean checkD = true;
						// do{
							// System.out.print("[반납일자] 년월일(yyyy-mm-dd): ");
							// rent_Return_Date = input.next();
					...
					dto = dao.check_ReturnBook(member_Id, book_Id);
					if(dto != null){
						...
						if(checkGetYN.equals("y") || checkGetYN.equals("Y") || checkGetYN.equals("ㅛ")){
							...
							System.out.println("양식에 따라 정확히 입력해주시기 바랍니다.");
						}
					}else{
						System.out.println("일치하는 대출정보가 없습니다. 다시 확인해주시기 바랍니다.");
					}
			}while(arr.size() == 0);
		}
	}
